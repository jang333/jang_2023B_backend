...
public class Step3 {
   ...
   */
    public static void main(String[] args) {
        ...
        //DB연동
        Connection connection = null; //try{ } 와 while{ } 에서 모두 사용하기위해 선언만 먼저 작성
        try {
            //1.JDBC 드라이버 로딩
            Class.forName("com.mysql.cj.jdbc.Driver");
            //2.DB서버와 연동 후 연동객체를 반환 받기
            ...
            //3.DB연동시 필요한 메소드 중에 Class.forName, getConnection 일반예외 발생
        ...
        while (true){//무한루프
            ...
            try {
                int ch = scanner.nextInt(); //만약 int타입 범위 벗어난 입력시 오류 발생[InputMismatchException]
                if (ch==1){
                    //1. 입력받기
                    System.out.println("저장할 고객명 : "); String name= scanner.next();
                    //2. DB에 저장하기
                    //connection.prepareStatement("insert into members values('유재석')").executeUpdate(); //SQLException 예외처리
                    //String sql = "insert into members values('입력받은값')";
                    String sql = "insert into members values('"+name+"')";
                    System.out.println(sql);
                    connection.prepareStatement(sql).executeUpdate();
                }else if(ch==2){
                    //모든 필드(*)를 표시하는 레코드(where 없이) 출력
                    ...
                    while (resultSet.next()){//검색결과내 첫번쨰 레코드부터 마지막 레코드까지 순회
